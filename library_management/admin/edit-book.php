...
$id = $_GET['id'];
$fetch_query = mysqli_query($conn, "select * from book where id='$id'");
...
if(isset($_REQUEST['save-book-btn']))
{
    ...
    $update_book = mysqli_query($conn,"update book set book_name='$book_name', category='$category_name', isbnno='$isbn', author='$author_name', publisher='$publisher_name', price='$price', quantity='$quantity', place='$location_name',  availability='$availability' where id='$id'");
 ...
 <option <?php if($rows['category_name']==$row['category']){ ?>
    ...
    <div class="col-lg-6">
        <select class="form-control" id="location_name" name="location_name" required>
            ...
            <?php 
             $fetch_category = mysqli_query($conn, "select * from location where status=1");
             while($rows = mysqli_fetch_array($fetch_category)){
            ?>
