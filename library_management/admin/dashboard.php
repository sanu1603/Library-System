...
if(empty($id))
{
 ...
 $select_user = mysqli_query($conn,"select count(*) from users where role=2");
 $total_user = mysqli_fetch_row($select_user);

 $select_book = mysqli_query($conn,"select count(*) from book");
 $total_book = mysqli_fetch_row($select_book);

 $select_avail_book = mysqli_query($conn,"select count(*) from book where availability=1");
 $avail_book = mysqli_fetch_row($select_avail_book);

 $issued_book = mysqli_query($conn,"select count(*) from issue where status=1");
 $issued_book = mysqli_fetch_row($issued_book);

 $return_book = mysqli_query($conn,"select count(*) from returned group by book_id");
